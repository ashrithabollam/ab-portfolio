          ...
          <div className="mt-8 flex md:flex-row justify-center flex-col md:items-stretch items-center ">
            {experienceData.map((el) => (
              <motion.div
                ...
                className={
                  theme.state.darkMode
                    ? "md:w-96 p-4 bg-white rounded-lg flex items-center flex-col mt-8 mx-4"
                    : "md:w-96 p-4 bg-gray-100 rounded-lg flex items-center flex-col mt-8 mx-4"
                }
              >
                <img src={el.img} alt="" />
                <h4 className="text-xl font-bold mt-4">{el.name}</h4>
                  <p className="text-md font-semibold mt-1 text-center">{el.company} — {el.location}</p>
                  <p className="text-sm italic text-center">{el.date}</p>
                  <p className="text-md mt-2 text-justify">{el.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
